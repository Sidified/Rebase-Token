...
# Placeholder for security - Do not push real private keys to GitHub
...
ZKSYNC_REBASE_TOKEN_ADDRESS=$(forge create src/RebaseToken.sol:RebaseToken --rpc-url ${ZKSYNC_SEPOLIA_RPC_URL} --account updraft --legacy --zksync --broadcast | awk '/Deployed to:/ {print $3}')
...
ZKSYNC_POOL_ADDRESS=$(forge create src/RebaseTokenPool.sol:RebaseTokenPool --rpc-url ${ZKSYNC_SEPOLIA_RPC_URL} --account updraft --legacy --zksync --broadcast --constructor-args ${ZKSYNC_REBASE_TOKEN_ADDRESS} '[]' ${ZKSYNC_RNM_PROXY_ADDRESS} ${ZKSYNC_ROUTER} | awk '/Deployed to:/ {print $3}')
...
# Set the permissions for the pool contract on Sepolia
echo "Setting the permissions for the pool contract on Sepolia..."
cast send ${SEPOLIA_REBASE_TOKEN_ADDRESS} "grantMintAndBurnRole(address)" ${SEPOLIA_POOL_ADDRESS} --rpc-url ${SEPOLIA_RPC_URL} --account updraft
echo "Pool permissions set"

# Set the CCIP roles and permissions on Sepolia
echo "Setting the CCIP roles and permissions on Sepolia..."
cast send ${SEPOLIA_REGISTRY_MODULE_OWNER_CUSTOM} "registerAdminViaOwner(address)" ${SEPOLIA_REBASE_TOKEN_ADDRESS} --rpc-url ${SEPOLIA_RPC_URL} --account updraft
cast send ${SEPOLIA_TOKEN_ADMIN_REGISTRY} "acceptAdminRole(address)" ${SEPOLIA_REBASE_TOKEN_ADDRESS} --rpc-url ${SEPOLIA_RPC_URL} --account updraft
cast send ${SEPOLIA_TOKEN_ADMIN_REGISTRY} "setPool(address,address)" ${SEPOLIA_REBASE_TOKEN_ADDRESS} ${SEPOLIA_POOL_ADDRESS} --rpc-url ${SEPOLIA_RPC_URL} --account updraft
echo "CCIP roles and permissions set"
...
cast send ${ZKSYNC_POOL_ADDRESS}  --rpc-url ${ZKSYNC_SEPOLIA_RPC_URL} --account updraft "applyChainUpdates(uint64[],(uint64,bytes[],bytes,(bool,uint128,uint128),(bool,uint128,uint128))[])" "[${SEPOLIA_CHAIN_SELECTOR}]" "[(${SEPOLIA_CHAIN_SELECTOR},[$(cast abi-encode "f(address)" ${SEPOLIA_POOL_ADDRESS})],$(cast abi-encode "f(address)" ${SEPOLIA_REBASE_TOKEN_ADDRESS}),(false,0,0),(false,0,0))]" --legacy --zksync

# Bridge the funds using the script to zksync 
echo "Bridging the funds using the script to ZKsync..."
SEPOLIA_BALANCE_BEFORE=$(cast erc20 balance ${SEPOLIA_REBASE_TOKEN_ADDRESS} $(cast wallet address --account updraft) --rpc-url ${SEPOLIA_RPC_URL})
...
SEPOLIA_BALANCE_AFTER=$(cast erc20 balance ${SEPOLIA_REBASE_TOKEN_ADDRESS} $(cast wallet address --account updraft) --rpc-url ${SEPOLIA_RPC_URL})
